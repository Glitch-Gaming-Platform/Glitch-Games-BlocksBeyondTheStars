...
Status: **analysis + plan (nothing implemented yet)**. Requested: build a **space-station editor** and a
**town/village editor** (like the ship editor), then make world generation pick from **lists of
hand-designed station/village/town types** *in addition to* procedural generation. Per instruction:
**analyse first** — this document is that analysis. Builds directly on the
[ship-type editor](SHIP_TYPE_EDITOR_PLAN.md) (same editor → export → merge → stamp pattern).
