...
> in [STORY_IMPLEMENTATION_PLAN.md](STORY_IMPLEMENTATION_PLAN.md); *what is built* in [../TODO.md](../TODO.md).
>
...
> storylines; the engine is pluggable (see [STORY_IMPLEMENTATION_PLAN.md](STORY_IMPLEMENTATION_PLAN.md) §2,
> D2–D4). **Authoring status:** §A/§B (canon), the §E schema and the §H starter texts exist; the **full
> corpus** (all beats B0–B12, net fragments per category, player memories, settler flavour, the finale
> arguments) **still needs writing out before it can be integrated** (plan workstream W-A).
>
...
>
> Last updated: 2026-06-14 (rev. 3 — authoritative SPS grundstory, clone twist, three machine types,
...
(§G). Destined for `data/lore.json` + locale files.
...
[STORY_IMPLEMENTATION_PLAN.md](STORY_IMPLEMENTATION_PLAN.md) P6).
