...
Status (2026-06-19): **IMPLEMENTED** — both requests are built (server/shared tests green; client phases
need a Unity build to verify). Request #1: tier-aware weighted pack-filtered template selection (P1),
runtime user-content ingestion so in-game builds appear in new worlds without a rebuild (P2), per-world
"which structures" creation UI (P3), and the editor **palette + dye/glow/shape/orient** upgrade (U1–U4,
§7). Request #2: dev-editor signposting — Material/Tech split into a "developer content tools" group +
in-editor banner (P4). See [../../TODO.md](../../TODO.md) for live status and the per-phase IMPLEMENTED notes in
§6 and §7 below. (This doc is retained as the analysis + locked decisions + file-touch record.) The two
requests were:
...
This builds on [SHIP_TYPE_EDITOR.md](SHIP_TYPE_EDITOR.md) and
[STATION_SETTLEMENT_EDITOR.md](STATION_SETTLEMENT_EDITOR.md), which are now **largely
...
- `data/ship_layouts/` ships 3 layouts (scout / corvette / hauler). *(Updated: `data/station_templates.json`
  and `data/settlement_templates.json` ARE now shipped (P1 sample templates) → the pools are non-empty by
  default. The "not shipped yet / empty pool" note below described the pre-P1 state.)*
...
([ItemKey](../../src/BlocksBeyondTheStars.Shared/State/ItemKey.cs)). The **only** missing links are
