...
Status: **analysis — superseded in large part by later render/UI work** (see
[PROFESSIONAL_LOOK_IMPLEMENTATION.md](PROFESSIONAL_LOOK_IMPLEMENTATION.md),
[ART_BIBLE.md](ART_BIBLE.md) and TODO.md).
Original analysis date: 2026-06-12.

> **Re-verified 2026-06-19 against the current client.** Many gaps below are now CLOSED; the per-row
> "Status" cells in §3 reflect the **original 2026-06-12 state** and are kept for history. The accurate
> current status is:
>
> **Now DONE (correct the §3 rows accordingly):**
> - **G1 Linear color space** — `ProjectSettings.asset` `m_ActiveColorSpace: 1` (Linear); sRGB-authored
>   constants are converted at the shader boundary via `ShaderColor.Srgb()`. (was MISSING)
> - **G4 event post FX** — `UrpScenePost.Burst`/`PulseVignette`/`SetOxygenAlarm` are now wired (ship damage,
>   space, low-O₂). (was MISSING) — only **G5 screenshot/photo mode** remains open.
> - **G8 block texture variants + random face rotation** — `BlockTextureAtlas.VariantKeys` (2 variants for
>   natural blocks) + 90° face rotation from a world-pos hash in `ChunkMesher`. (was PARTIAL)
> - **G11 heat shimmer** — `HeatShimmer.cs` + `HeatHaze.shader` (opaque/depth warp, depth-faded, hot-world
>   gated). (was MISSING)
> - **G12 block light at night** — baked coloured flood-fill with directional N·L + specular in
>   `BlockAtlas.shader` (TEXCOORD3/4); glow blocks light their surroundings. (was PARTIAL)
> - **G13 + G16 ship room identity** — `GameServerShipStructure.PaintStationAccents` + `StationDecorView`:
>   per-room floor accents (medbay/lab/cargo/engine/workshop), `strip_light_cyan` corridor strips, per-room
>   decor + emergency lighting state. (were PARTIAL)
> - **G14 cockpit** — `StationDecorView` builds a console base + animated screen (`ScreenFlicker`) + a
>   holographic star map (`HoloMap`). (was MISSING)
> - **G17 ship damage visuals** — `ShipDamageView.cs`: spark bursts <50% hull, red emergency light + alarm
>   <25% (with hysteresis). (was MISSING 0%)
> - **G19 launch/landing dust + thruster glow** — `ShipTransitView` DustBurst + under-hull thruster glow.
>   (was PARTIAL)
> - **G22 final-hit flash + fly-to-hotbar pickup** — `MiningFx.FlyPickup` + final-hit flash. (was MISSING)
> - **G23 low-oxygen warning** — `HudUi.UpdateOxygenAlarm` pulsing blue vignette + escalating beep under 25%.
>   (was MISSING)
> - **G27 sci-fi font** — primary `Resources/fonts/Rajdhani-Medium` (OFL), LegacyRuntime only as fallback.
>   (was PARTIAL) — note the UI is now **uGUI** (M27 rework), not the IMGUI of the 2026-06-12 analysis.
> - **G29 music contexts** — `ClientMusic` director with ~2.5 s crossfades + a selectable AI-composed track
>   library mapped to contexts. (was PARTIAL)
> - **G32 Art Bible** — written: [ART_BIBLE.md](ART_BIBLE.md) (normative, named palette + per-object
>   quality checklist). (was PARTIAL)
>
> **Still open / partial (largely as listed, tracked in ADVANCED_GRAPHICS phases 2–4):** G2 reflection
> probes, G3 LOD/greedy meshing, G5 screenshot mode, G6 decals, G7 detail scatter, G9 biome blending,
> G10 (reduced — many ship-block strips/hazard blocks now exist via the room pass), G24 cinematic camera,
> G30 3D preview icons, G31 sky meteors/distant ships.
...
Related docs: [ADVANCED_GRAPHICS.md](ADVANCED_GRAPHICS.md) (phases 2–4 overlap with many
...
What was missing (as of the 2026-06-12 analysis) clustered in four areas: **(a) one config gap (then Gamma;
since switched to Linear — see the re-verify note above), (b) "juice"/feedback effects (crafting,
blueprints, pickup, cinematic camera moments), (c) ship interior/cockpit presentation (rooms, displays,
holograms, damage states, build preview), and (d) world detail layers (decals, detail props, LOD,
reflection probes, heat distortion).** Clusters (a)–(c) are now largely closed (Linear, juice, rooms,
cockpit, damage); cluster (d) is the main remaining work (decals, scatter, LOD, reflection probes — heat
distortion itself is now done).
...
dual-pipeline `.shader` files; see the constraints section of ADVANCED_GRAPHICS.md).
...
| G2 | **Reflection probes** | §2, §13 | **MISSING** | No probes anywhere; metal/glass reflect only a fresnel sky-color approximation in `BlockAtlas.shader`. Interiors (ship, station) are where probes would pay off. Already listed as ADVANCED_GRAPHICS Phase 4/J. |
...
| # | Gap | Plan ref | Status | Details |
|---|---|---|---|---|
| G6 | **Decal layer** (dirt, scratches, logos, moss, scorch marks, impact craters) | §6, §7.2, §9.2 | **MISSING** | No decal system of any kind (URP Decal Projector unused). Also blocks the "used/worn ship" look (§9.2 Gebrauchsspuren) and weapon scorch feedback. ADVANCED_GRAPHICS item I. |
| G7 | **Detail props / scatter** (pebbles, grass tufts, small rocks on surfaces) | §7.2 | **MISSING** | Only cross-quad flora billboards exist. No instanced scatter (`DrawMeshInstanced` etc.). ADVANCED_GRAPHICS item E + shells (Phase 2). |
...
**Large / strategic (already tracked in ADVANCED_GRAPHICS phases 2–4)**
...
- **ADVANCED_GRAPHICS.md header is stale:** it still says "Built-in RP, no URP today" while
