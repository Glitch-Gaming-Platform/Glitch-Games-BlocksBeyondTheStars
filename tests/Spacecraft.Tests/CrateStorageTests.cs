...
public sealed class CrateStorageTests : IDisposable
{
    ...
    {
        _root = Path.Combine(Path.GetTempPath(), "spacecraft_crate_" + Guid.NewGuid().ToString("N"));
    ...
    private static SvGameServer WithBuilder(SvGameServer server, out Spacecraft.Shared.State.PlayerState state)
    {
