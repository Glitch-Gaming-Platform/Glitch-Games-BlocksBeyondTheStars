...
public sealed class PlaceableDoorTests : IDisposable
{
    ...
    {
        _root = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "spacecraft_door_" + Guid.NewGuid().ToString("N"));
    ...
    private static Spacecraft.GameServer.PlayerSession Builder(SvGameServer server)
    {
