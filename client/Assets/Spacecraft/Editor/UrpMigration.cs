...
{
    /// <summary>
    /// URP migration helpers (menu: <b>Spacecraft → URP</b>). Enables/disables URP unambiguously by assigning the
    ...
    {
        [MenuItem("Spacecraft/URP/Enable URP")]
        public static void EnableUrp()
        {
            ...
            Debug.Log($"[URP] ENABLED. Effective pipeline = {PipelineName()} (asset: {AssetDatabase.GetAssetPath(asset)}). " +
                      "Enter Play mode to view. Run 'Spacecraft → URP → Disable URP' to revert to Built-in.");
        }

        [MenuItem("Spacecraft/URP/Disable URP (Built-in)")]
        ...
        [MenuItem("Spacecraft/URP/Log Active Pipeline")]
