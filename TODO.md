...
- **Phase 3 (water) — depth-aware water ✅ (URP path):** `BlockAtlasTransparent.shader` reads scene depth → shallow→deep colour gradient (deep sea turns opaque/blue, hides the bed) + automatic foam line where geometry breaks the surface (shores/rocks/swimmers).
...
- **⏭ Open:** **weather** VFX is deliberately NOT migrated — `WeatherFx3D` is world-aware (per-column sky checks, dies on roofs/in caves, ship roofing); a naïve ParticleSystem would regress that (rain in caves / through roofs). Also open: **warp** VFX (IMGUI star-streaks → 3D particles), URP decals; Phase 3 **SSR** renderer feature + water refraction; the **volumetric god-rays** as an AFTER-post pass (the darkening-safe redesign).
